...
# racadm remoteimage -c -u admin -p xxx -l '//192.168.0/dev/floppy.img'
sshpass -p $PASS ssh -n -o StrictHostKeyChecking=no -l $USER $1 racadm remoteimage -c -u $USER -p $PASS  -l '192.168.0.1:/mnt/home/centos7.iso' 
sshpass -p $PASS ssh -n -o StrictHostKeyChecking=no -l $USER $1 racadm set iDRAC.VirtualMedia.BootOnce 1 
sshpass -p $PASS ssh -n -o StrictHostKeyChecking=no -l $USER $1 racadm set iDRAC.ServerBoot.FirstBootDevice VCD-DVD
